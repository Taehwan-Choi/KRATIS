...
class MapTrackingService : Service() {
    ...
    data class LocationInfo(val time: Long,  // 측정시간(ms)
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult?) {
                ...
                for (location in locationResult.locations) {
                    ...
                    currentlocationInfo = LocationInfo(location.time,
                                                       location.latitude,
                                                       location.longitude,
...
//                        latLngList.add(LatLng(location.latitude, location.longitude))
                        ...
                        latLngList.add(LatLng(currentlocationInfo.latitude,
                        ...
                        if (isInStartArea(currentlocationInfo.latitude,
                                          currentlocationInfo.longitude)
                        ) {
                            startTime = Date()
                        }
                        ...
                        if (startTime != null) {
                            if (isInFinishArea(currentlocationInfo.latitude,
                                               currentlocationInfo.longitude)
                            ) {
...
//                        makingLog("위치 측정 부정확(오차 ${location.accuracy.toInt()} m)")
    ...
    private fun isInStartArea(latitude: Double, longitude: Double): Boolean {
        ...
        val targetBounds =
            LatLngBounds.Builder().include(startBottomLeft).include(startTopRight).build()
    ...
    private fun isInFinishArea(latitude: Double, longitude: Double): Boolean {
        val targetBounds =
            LatLngBounds.Builder().include(finishBottomLeft).include(finishTopRight).build()
    ...
    private fun createNotification(): Notification {
        ...
        val notification =
            NotificationCompat.Builder(this, CHANNEL_ID).setContentTitle("Tracking Service")
